...
namespace ifb {
    ...
    renderer_quad_shader_init(
        ...
        auto& shdr = _renderer_ctx->quad_shader;
        
        // commit memory
        // NOTE(SAM): we can optimize this to use one block
        void* buf_mem_vtx  = renderer_memory_commit(_renderer_ctx);
        void* buf_mem_elmt = renderer_memory_commit(_renderer_ctx); 
        assert(
            buf_mem_vtx  != NULL &&
            buf_mem_elmt != NULL
        );
        ...
        shdr.vtx_buffer.capacity = renderer_memory_element_count(_renderer_ctx, sizeof(quad_vertex));
        shdr.vtx_buffer.elements = (quad_vertex*)buf_mem_vtx;
       
        // initialize element buffer
        shdr.elmnt_buffer.count    = 0;
        shdr.elmnt_buffer.capacity = renderer_memory_element_count(_renderer_ctx, sizeof(u32));
        ...
        const u32 vertex_data_size  = _renderer_ctx->mem.granularity;
        byte*     element_data_ptr  = (byte*)shdr.elmnt_buffer.elements;
        const u32 element_data_size = _renderer_ctx->mem.granularity; 
        ...
        gl_ok &= gl_buffer_set_vertex_data     (_renderer_ctx->gl, shdr.gl.buf_vertex,  vertex_data_ptr,  vertex_data_size);
        gl_ok &= gl_buffer_set_element_data    (_renderer_ctx->gl, shdr.gl.buf_element, element_data_ptr, element_data_size);
    ...
    renderer_quad_push(
        ...
        assert(
            _renderer_ctx                                  != NULL                                 &&
            _renderer_ctx->quad_shader.gl.program          != GL_ID_INVALID                        && 
            _renderer_ctx->quad_shader.gl.vertex           != GL_ID_INVALID                        && 
            _renderer_ctx->quad_shader.gl.buf_vertex       != GL_ID_INVALID                        && 
            _renderer_ctx->quad_shader.gl.buf_element      != GL_ID_INVALID                        &&
            _renderer_ctx->quad_shader.vtx_buffer.capacity != 0                                    &&
            _renderer_ctx->quad_shader.vtx_buffer.count    <= _renderer_ctx->quad_shader.vtx_buffer.capacity &&
            _renderer_ctx->quad_shader.vtx_buffer.elements != NULL
        );
