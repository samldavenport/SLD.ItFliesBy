...
namespace ifb {
    ...
    gui_renderer_camera(
        ...
        if (ImGui::Begin(str->window_renderer_camera, &is_open)) {
            ...
            renderer_camera_get_origin (_eng_context->renderer, camera_target);
            renderer_camera_get_target (_eng_context->renderer, camera_origin);

            gui_control_vec3 (str->camera_origin, camera_target);
            gui_control_vec3 (str->camera_target, camera_origin);
